...
Each test: 2400 steps (8 episodes x 300), ~10-15s per test.
"""
...
def run_parallel(n, device="cpu", threads_per_proc=None):
    ...
    for i in range(n):
        cmd = [sys.executable, "train.py",
            "--algorithm", "AB-MAPPO", "--num_mus", str(K), "--num_uavs", str(M),
            "--total_steps", str(STEPS), "--episode_length", str(EPL),
            "--seed", str(SEED+i), "--device", device,
            "--disable_tensorboard", "--run_dir", f"/tmp/bp_{device}_{n}_{i}"]
        procs.append(subprocess.Popen(cmd, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE))
    for p in procs:
        p.wait()
