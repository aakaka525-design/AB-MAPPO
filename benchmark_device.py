...
def run_bench(label, device, blas_threads=None):
    ...
    cmd = [
        ...
        "--algorithm", "AB-MAPPO",
        "--num_mus", str(K), "--num_uavs", str(M),
        "--total_steps", str(STEPS),
        "--episode_length", str(EPL),
        "--seed", str(SEED),
        "--device", device,
        "--disable_tensorboard",
        "--run_dir", run_dir,
    ...
    # extract eps/s from last episode line
    eps_s = "?"
...
times = {r[0]: r[1] for r in results}
